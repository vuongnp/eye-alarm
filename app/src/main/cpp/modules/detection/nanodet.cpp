...
NanoDet::NanoDet(json detection_cfg)
{
    mnn_path = detection_cfg["mnn_path"].get<std::string>();
    input_size = detection_cfg["input_size"].get<int>();
    num_thread = detection_cfg["num_thread"].get<int>();
    labels = detection_cfg["labels"].get<std::vector<std::string>>();
    input_name = detection_cfg["input_name"].get<std::string>();
    output_name = detection_cfg["output_name"].get<std::string>();
    num_class = labels.size();
    score_threshold =  detection_cfg["score_threshold"].get<float>();
    nms_threshold = detection_cfg["nms_threshold"].get<float>();
...
void NanoDet::detect(cv::Mat &raw_image, std::vector<BoxInfo> &result_list)
{
    if (raw_image.empty()) {
    ...
    interpreter->resizeTensor(input_tensor, {1, 3, input_size, input_size});
    interpreter->resizeSession(session);
    std::shared_ptr<MNN::CV::ImageProcess> pretreat(
        MNN::CV::ImageProcess::create(MNN::CV::BGR, MNN::CV::BGR, mean_vals, 3,
                                        norm_vals, 3));
    pretreat->convert(image.data, input_size, input_size, image.step[0], input_tensor);
    ...
    MNN::Tensor *tensor_preds = interpreter->getSessionOutput(session, output_name.c_str());
    ...
    for (int i = 0; i < (int)results.size(); i++)
    {
        ...
        {
            box.x1 = box.x1 / input_size * image_w;
            box.x2 = box.x2 / input_size * image_w;
            box.y1 = box.y1 / input_size * image_h;
            box.y2 = box.y2 / input_size * image_h;
...
BoxInfo NanoDet::disPred2Bbox(const float *&dfl_det, int label, float score, int x, int y, int stride)
{
    ...
    float xmax = (std::min)(ct_x + dis_pred[2], (float)input_size);
    float ymax = (std::min)(ct_y + dis_pred[3], (float)input_size);
